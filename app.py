...
flask_server.secret_key = Config.FLASK_SECRET_KEY
...
prefix = Config.ROOT_PATH_PREFIX.rstrip("/") + "/"
...
def serve_layout() -> Any:
    """
    Layout base do Dash, incluindo MantineProvider e placeholders para roteamento.
    ...
    return dmc.MantineProvider(
        ...
        children=[
            ...
            dcc.Store(id="login-status", storage_type="session", data={"logged_in": False, "token": None}),
...
# Registra callbacks após o app existir
register_callbacks()
...
def render_page(pathname: str, login_status: dict) -> Any:
    ...
    - Se a rota for protegida e o usuário não estiver autenticado, exibe o login.
    ...
    prefix_clean = Config.ROOT_PATH_PREFIX.rstrip("/")
    protected_routes = {
        f"{prefix_clean}/home",
        f"{prefix_clean}/main",
        f"{prefix_clean}/meus-dados",
    }
