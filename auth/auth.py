...
def _save_tokens_to_session(token_bundle: Dict[str, Any]) -> None:
    ...
    refresh_expires_in = int(token_bundle.get("refresh_expires_in", 0))  # ex.: 1800
    ...
    # Info útil para UI
    session["username"] = claims.get("preferred_username") or claims.get("email") or ""
...
def check_credentials(
    ...
    try:
        ...
        home_path = f"{Config.ROOT_PATH_PREFIX.rstrip('/')}/home"
...
def handle_logout(n_clicks: int):
    ...
    login_path = f"{Config.ROOT_PATH_PREFIX.rstrip('/')}/login"
